                    ...
                    for k in K:
                        for r in muR:
                            ...
                            print("---- lambAI: %d, lambBI: %d, lambIA: %d, lambIB: %d"
                                  "queue size: %d, B: %d, simtime: %d ----" % (ai, bi, ia, ib, k, B, simtime))
                            ...
                            Wk = 1.0 * ai / (r-ai-ia) + 1.0 * ai / (muL-ai) + 1.0 * bi / (r-bi-ib) \
                                + 1.0 * bi / (muL-bi) + 2.0 * lambABI / (r-lambABI-lambIBA) + 2.0 * lambABI / (muL-lambABI)

                            WkIA = 2.0 * lambIBA / (r-lambIBA-lambABI) + 1.0 * lambIBA / (muL-lambIBA) + 2.0 * ia / (muL-ia) \
                                   + 1.0 * ia / (r-ia-ai)

                            WkIB = 2.0 * lambIBA / (r-lambIBA - lambABI) + 1.0 * lambIBA / (muL-lambIBA) + 2.0 * ib / (muL-ib) \
                                   + 1.0 * ib / (r-ib - bi)
                            ...
                            array.append({'lambdaAI': ai,
                                          ...
                                          'Average Delay ABI': round(average_delay, 5),
                                          'Average Delay IA': round(average_delayIA, 5),
                                          'Average Delay IB': round(average_delayIB, 5),
                                          'Transmitted bandwidth ABI': round(trans_band, 5),
                                          'Transmitted bandwidth IA': round(trans_bandIA, 5),
                                          'Transmitted bandwidth IB': round(trans_bandIB, 5)})
